...
// 获取头部
func (db *SkipList) head() int {
	return nKV
}

func (db *SkipList) randLevel() int {
	level := 1
...
func (db *SkipList) findGreaterOrEqual(key []byte, prev bool) (int, bool) {
	node := 0
	level := db.level
	for i := level - 1; i >= 0; i-- {
...
func (db *SkipList) nextKey(nodeDataOffset int, level int) []byte {
	return db.key(nodeDataOffset + nNext + level)
}

// 返回nodeData当前level的下一个value
func (db *SkipList) nextValue(nodeDataOffset int, level int) []byte {
	return db.value(nodeDataOffset + nNext + level)
}

// 当前节点所在的层的下一个节点
func (db *SkipList) next(nodeDataOffset, level int) int {
	return db.nodeData[nodeDataOffset+nNext+level]
}

// 当前节点所在的层是否有下一个节点
func (db *SkipList) hasNext(nodeDataOffset, level int) bool {
	return db.next(nodeDataOffset, level) != 0
}

// Put 将key, value放入到db中
func (db *SkipList) Put(key, value []byte) {
	db.rwMutex.Lock()
...
func (db *SkipList) Delete(key []byte) bool {
...
// Contains 是否存在该key
func (db *SkipList) Contains(key []byte) bool {
...
// Get 查询key -> value
func (db *SkipList) Get(key []byte) ([]byte, bool) {
